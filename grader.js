...
var checkHtmlFromUrl = function(buffer, checksfile) {
  $ = cheerio.load(buffer);
  return checkHtml($, checksfile);
}
...
var getHtml = function(url) {
  rest.get(url).on('complete', function(result) {
    if (result instanceof Error) {
      console.log('Error retriving data from ' + url + " : " + result.message);
      process.exit(1);
    } else {
      var checkJson = checkHtmlFromUrl(result, program.checks);
...
if(require.main == module) {
  program
    ...
    .parse(process.argv);
  if(program.url) getHtml(program.url);
  else {
